var __vueify_insert__ = require("vueify/lib/insert-css")
var __vueify_style__ = __vueify_insert__.insert("*[_v-2e2ce392] {\n  font-weight: 300;\n}\n[_v-2e2ce392]::-webkit-scrollbar {\n  display: none;\n}\n.xin-widget-citys[_v-2e2ce392] {\n  -webkit-transition: all 0.2s ease-out;\n  transition: all 0.2s ease-out;\n  position: fixed;\n  top: 0;\n  bottom: 0;\n  left: 0;\n  right: 0;\n  background: #fff;\n  z-index: 10000;\n  color: #333;\n  overflow-y: scroll;\n  -webkit-overflow-scrolling: touch;\n  text-align: left;\n  box-shadow: 1rem 1rem rgba(0, 0, 0, 0.2);\n  /***** 1px border *****/\n}\n.xin-widget-citys .xin-widget-citys-content[_v-2e2ce392] {\n  width: 100%;\n  height: auto;\n  background: #fff;\n  border-radius: 0.3rem;\n}\n.xin-widget-citys .xin-widget-citys-content .xin-widget-citys-header[_v-2e2ce392] {\n  height: 3rem;\n  display: -webkit-box;\n  background: #fafafa;\n}\n.xin-widget-citys .xin-widget-citys-content .xin-widget-citys-header .xin-widget-citys-iptbox[_v-2e2ce392] {\n  -webkit-box-flex: 1.0;\n  height: 100%;\n  padding: 0 1.2rem;\n  border: 1px solid #f3f2f2;\n}\n.xin-widget-citys .xin-widget-citys-content .xin-widget-citys-header .xin-widget-citys-iptbox input[_v-2e2ce392] {\n  width: 100%;\n  height: 100%;\n  /*line-height: 0.38rem;*/\n  display: block;\n  border: none;\n  background: none;\n  box-sizing: border-box;\n}\n.xin-widget-citys .xin-widget-citys-content .xin-widget-citys-header .xin-widget-citys-iptbox input[_v-2e2ce392]::-webkit-input-placeholder {\n  /*color: #cdcdcd;*/\n  text-align: left;\n}\n.xin-widget-citys .xin-widget-citys-content .xin-widget-citys-header a[_v-2e2ce392] {\n  width: 5rem;\n  height: 100%;\n  padding-top: 0.7rem;\n  /*line-height: 0.38rem;*/\n  text-align: center;\n  color: #999999;\n  display: block;\n}\n.xin-widget-citys .xin-widget-citys-content .xin-widget-citys-local[_v-2e2ce392] {\n  height: 3rem;\n  line-height: 3rem;\n  text-indent: 1.2rem;\n}\n.xin-widget-citys .xin-widget-citys-content .xin-widget-citys-list[_v-2e2ce392] {\n  position: relative;\n}\n.xin-widget-citys .xin-widget-citys-content .xin-widget-citys-list dl dt[_v-2e2ce392] {\n  line-height: 2rem;\n  color: #878787;\n  background: #f0f0f0;\n  text-indent: 0.5rem;\n}\n.xin-widget-citys .xin-widget-citys-content .xin-widget-citys-list dl dt em[_v-2e2ce392] {\n  position: relative;\n  top: 0.1rem;\n  margin-right: 6px;\n}\n.xin-widget-citys .xin-widget-citys-content .xin-widget-citys-list dl dd[_v-2e2ce392] {\n  padding: 0 1rem;\n  height: 2.3rem;\n  line-height: 2.3rem;\n}\n.xin-widget-citys .xin-widget-citys-searchlist[_v-2e2ce392] {\n  position: absolute;\n  top: 4rem;\n  width: 100%;\n  bottom: 0;\n  z-index: 10001;\n  background: #fff;\n  overflow: auto;\n}\n.xin-widget-citys .xin-widget-citys-searchlist li[_v-2e2ce392] {\n  margin: 0 1.6rem;\n  height: 2.5rem;\n  line-height: 2.5rem;\n  /*line-height: 0.5rem;*/\n}\n.xin-widget-citys .xin-widget-citys-searchlist .nomatch[_v-2e2ce392] {\n  /*line-height: 0.5rem;*/\n  text-align: center;\n  color: #aaa;\n}\n.xin-widget-citys .bdt[_v-2e2ce392],\n.xin-widget-citys .bdb[_v-2e2ce392],\n.xin-widget-citys .bdtb[_v-2e2ce392],\n.xin-widget-citys .bdl[_v-2e2ce392],\n.xin-widget-citys .bdr[_v-2e2ce392],\n.xin-widget-citys .bdlr[_v-2e2ce392] {\n  position: relative;\n}\n.xin-widget-citys .bdl[_v-2e2ce392]:before,\n.xin-widget-citys .bdr[_v-2e2ce392]:after,\n.xin-widget-citys .bdlr[_v-2e2ce392]:before,\n.xin-widget-citys .bdlr[_v-2e2ce392]:after,\n.xin-widget-citys .bdt[_v-2e2ce392]:before,\n.xin-widget-citys .bdb[_v-2e2ce392]:after,\n.xin-widget-citys .bdtb[_v-2e2ce392]:before,\n.xin-widget-citys .bdtb[_v-2e2ce392]:after {\n  background-size: 100% 100%;\n  position: absolute;\n  content: '';\n  z-index: 10;\n}\n.xin-widget-citys .bdl[_v-2e2ce392]:before,\n.xin-widget-citys .bdr[_v-2e2ce392]:after,\n.xin-widget-citys .bdlr[_v-2e2ce392]:before,\n.xin-widget-citys .bdlr[_v-2e2ce392]:after {\n  top: 0;\n  width: 1px;\n  height: 100%;\n}\n.xin-widget-citys .bdt[_v-2e2ce392]:before,\n.xin-widget-citys .bdb[_v-2e2ce392]:after,\n.xin-widget-citys .bdtb[_v-2e2ce392]:before,\n.xin-widget-citys .bdtb[_v-2e2ce392]:after {\n  left: 0;\n  height: 1px;\n  width: 100%;\n}\n.xin-widget-citys .bdb[_v-2e2ce392]:after {\n  background-image: url('data:image/png;base64,');\n  bottom: 0;\n}\n.xin-widget-citys-letnav[_v-2e2ce392] {\n  position: fixed;\n  right: 0;\n  bottom: 0;\n  top: 0.48rem;\n  padding-right: 0.26rem;\n  z-index: 10002;\n  display: -webkit-box;\n  -webkit-box-align: center;\n}\n.xin-widget-citys-letnav section[_v-2e2ce392] {\n  -webkit-box-flex: 1.0;\n}\n.xin-widget-citys-letnav ol[_v-2e2ce392] {\n  text-align: right;\n  padding-bottom: 0.48rem;\n}\n.xin-widget-citys-letnav ol li[_v-2e2ce392] {\n  padding: 0.03rem 0.02rem;\n  color: #666666;\n  width: 100%;\n  line-height: 100%;\n  box-sizing: border-box;\n  text-align: center;\n}\n.slide-enter[_v-2e2ce392],\n.slide-leave-active[_v-2e2ce392] {\n  -webkit-transform: translateX(100%);\n          transform: translateX(100%);\n}\n.fade-enter[_v-2e2ce392],\n.fade-leave-active[_v-2e2ce392] {\n  opacity: 0;\n}\n.star-big[_v-2e2ce392] {\n  width: 1rem;\n  height: 1rem;\n  display: inline-block;\n  background-size: cover;\n  background-image: url('data:image/png;base64,');\n}\n.star-small[_v-2e2ce392] {\n  width: 1.2rem;\n  height: 1.2rem;\n  display: inline-block;\n  background-size: cover;\n  background-image: url('data:image/png;base64,');\n}\n")
'use strict';

Object.defineProperty(exports, "__esModule", {
    value: true
});

var _stringify = require('babel-runtime/core-js/json/stringify');

var _stringify2 = _interopRequireDefault(_stringify);

require('./nomalize');

require('./responsive');

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

exports.default = {
    props: {
        isShow: {
            type: Boolean,
            default: false,
            required: false
        },
        simple: {
            type: Boolean,
            default: false
        },
        localCity: {
            type: Object
        },
        starCity: {
            type: Array
        },
        cityData: {
            type: Array
        },
        onChoose: {
            type: Function
        },
        initCity: {
            type: Function,
            default: null
        },

        close: {
            type: Function
        }
    },
    data: function data() {
        return {
            input: '',
            list: '',
            targetLetter: '',
            searchList: [],
            hotList: [],
            letterList: [],
            result: '',
            defaultTrigger: true
        };
    },
    watch: {
        cityData: function cityData() {
            if (this.cityData && this.cityData.length > 0) {
                this._formatCityList(this.cityData);
            }
        },
        input: function input() {
            this._search();
        },
        targetLetter: function targetLetter() {
            this._scrollView();
        }
    },
    activate: function activate(done) {
        var _this = this;

        done();
        if (this.cityData && this.cityData.length > 0) {
            this._formatCityList(this.cityData);
        }
    },
    methods: {
        show: function show() {},
        hide: function hide() {
            this.input = '';
            this.searchList = [];
            this.close && this.close();
        },
        _chooseDefault: function _chooseDefault() {
            var _this = this;
            var arr = this.list.filter(function (item) {
                return _this.localCity.cityId == item.cityId;
            });
            if (arr.length > 0) {
                _this._chooseOne(arr[0]);
            } else {
                _this._chooseOne(this.list[0]);
            }
        },

        _chooseLetter: function _chooseLetter(e) {
            var symbol = e.target.getAttribute('data-type');

            if (symbol == 'starCity') {
                this.targetLetter = '热门城市';
            } else {
                this.targetLetter = e.target.innerText;
            }
        },

        _touchLetters: function _touchLetters(e) {
            e.preventDefault();

            var ol = document.querySelector('.xin-widget-citys-letnav ol'),
                liNum = document.querySelectorAll('.xin-widget-citys-letnav li').length,
                olHei = ol.clientHeight,
                liHei = Math.round(olHei / liNum),
                olTop = ol.offsetTop + ol.offsetParent.offsetTop,
                olBot = olTop + olHei,
                touchY = e.touches[0].pageY,
                $wrapper = document.querySelector('.xin-widget-citys');

            switch (e.type) {
                case 'touchstart':
                    if (touchY < olTop) {
                        $wrapper.scrollTop = 0;
                    } else if (touchY > olBot) {
                        $wrapper.scrollTop = 10000;
                    }
                    break;
                case 'touchmove':
                    if (touchY < olBot && touchY > olTop) {
                        var olTouchY = touchY - olTop,
                            targetIndex = Math.ceil(olTouchY / liHei),
                            target = document.querySelectorAll('.xin-widget-citys-letnav li')[targetIndex - 1];

                        if (target.getAttribute('data-type') == 'starCity') {
                            this.targetLetter = '热门城市';
                        } else {
                            this.targetLetter = target.innerText;
                        }
                    } else if (touchY <= olTop) {
                        $wrapper.scrollTop = 0;
                    } else {
                        $wrapper.scrollTop = 10000;
                    }
                    break;
            }
        },

        _scrollView: function _scrollView() {
            var dtList = document.querySelectorAll('.xin-widget-citys-list dt'),
                _this = this,
                _dtList = Array.prototype.slice.call(dtList);

            if (_dtList) {
                _dtList.forEach(function (value, index, array) {
                    if (value.innerText === _this.targetLetter) {
                        var wrapper = document.querySelector('.xin-widget-citys');
                        var scrollTop = document.querySelector('.xin-widget-citys-list').offsetTop + value.offsetTop;

                        wrapper.scrollTop = scrollTop;

                        return false;
                    }
                });
            }
        },

        _search: function _search() {
            var reg = new RegExp(this.input == '' ? 'xxyy' : this.input, 'ig');

            var _arr = [];
            for (var i in this.letterList) {
                for (var j = 0; j < this.letterList[i].length; j++) {
                    if (reg.test(this.letterList[i][j]['cityName']) || reg.test(this.letterList[i][j]['cityFirstLetter']) || reg.test(this.letterList[i][j]['citySpell'])) {
                        _arr.push(this.letterList[i][j]);
                    }
                }
            }
            this.searchList = _arr;
        },

        _formatCityList: function _formatCityList(arr) {
            var letterArr = {};

            if (this.simple) {
                for (var i = 0; i < arr.length; i++) {
                    letterArr[i] = [];
                    letterArr[i].push(arr[i]);
                }
            } else {
                if (this.starCity && this.starCity.length > 0) {
                    var _starCity = this.starCity;

                    _starCity.forEach(function (value, index, array) {
                        if (!('star' in letterArr)) {
                            letterArr['star'] = [];
                            letterArr['star'].unshift(value);
                        } else {
                            letterArr['star'].unshift(value);
                        }
                    });
                }
                for (var i = 0; i < arr.length; i++) {
                    if (!(arr[i]['cityFirstLetter'] in letterArr)) {
                        letterArr[arr[i]['cityFirstLetter']] = [];
                        letterArr[arr[i]['cityFirstLetter']].push(arr[i]);
                    } else {
                        letterArr[arr[i]['cityFirstLetter']].push(arr[i]);
                    }
                }
            }
            this.letterList = letterArr;
        },
        _chooseOne: function _chooseOne(obj) {
            this.onChoose && this.onChoose(JSON.parse((0, _stringify2.default)(obj)));
        }
    }
};
if (module.exports.__esModule) module.exports = module.exports.default
;(typeof module.exports === "function"? module.exports.options: module.exports).template = "\n\t<transition name=\"slide\" _v-2e2ce392=\"\">\n    <div class=\"xin-widget-citys animated\" v-if=\"isShow\" _v-2e2ce392=\"\">\n        <div class=\"xin-widget-citys-content\" _v-2e2ce392=\"\">\n            <div class=\"xin-widget-citys-header bdb\" _v-2e2ce392=\"\">\n                <div class=\"xin-widget-citys-iptbox\" _v-2e2ce392=\"\">\n                    <input type=\"text\" placeholder=\"城市中文名或拼音\" v-model=\"input\" _v-2e2ce392=\"\">\n                </div>\n                <a href=\"javascript:void(0)\" @click=\"hide\" _v-2e2ce392=\"\">取消</a>\n            </div>\n            <div class=\"xin-widget-citys-local bdb\" _v-2e2ce392=\"\">当前城市：{{localCity.cityName || \"无法定位当前城市\"}}</div>\n            <div class=\"xin-widget-citys-list\" v-if=\"input == ''\" _v-2e2ce392=\"\">\n                <dl _v-2e2ce392=\"\">\n                    <template v-for=\"(item, index) in letterList\">\n                        <template v-if=\"index == 'star'\">\n                            <dt _v-2e2ce392=\"\"><em class=\"star-big\" _v-2e2ce392=\"\"></em>热门城市</dt>\n                        </template>\n                        <template v-else=\"\">\n                            <dt v-if=\"isNaN(index)\" _v-2e2ce392=\"\">{{index}}</dt>\n                        </template>\n                        <template v-for=\"(item2, key) in item\">\n                            <dd class=\"bdb\" @click=\"_chooseOne(item2)\" _v-2e2ce392=\"\">{{item2.cityName}}</dd>\n                        </template>\n                    </template>\n                </dl>\n            </div>\n            <div class=\"xin-widget-citys-searchlist\" v-if=\"input !== ''\" _v-2e2ce392=\"\">\n                <ul v-if=\"searchList.length!==0\" _v-2e2ce392=\"\">\n                    <li class=\"bdb\" v-for=\"item in searchList\" @click=\"_chooseOne(item)\" _v-2e2ce392=\"\">{{item.cityName}}</li>\n                </ul>\n                <div v-else=\"\" class=\"nomatch\" _v-2e2ce392=\"\">没有匹配城市</div>\n            </div>\n        </div>\n\t\t<div class=\"xin-widget-citys-letnav\" v-if=\"isShow &amp;&amp; input == '' &amp;&amp; !simple\" @touchmove=\"_touchLetters\" _v-2e2ce392=\"\">\n\t        <ol _v-2e2ce392=\"\">\n\t            <template v-for=\"(item, index) in letterList\">\n\t                <li v-if=\"isNaN(index) &amp;&amp; index=='star'\" @click=\"_chooseLetter\" data-type=\"starCity\" _v-2e2ce392=\"\"><em class=\"star-small\" data-type=\"starCity\" _v-2e2ce392=\"\"></em></li>\n\t                <li v-else=\"\" @click=\"_chooseLetter\" data-type=\"letter\" _v-2e2ce392=\"\">{{index}}</li>\n\t            </template>\n\t        </ol>\n\t    </div>\n    </div>\n\t</transition>\n"
if (module.hot) {(function () {  module.hot.accept()
  var hotAPI = require("vue-hot-reload-api")
  hotAPI.install(require("vue"), true)
  if (!hotAPI.compatible) return
  module.hot.dispose(function () {
    __vueify_insert__.cache["*[_v-2e2ce392] {\n  font-weight: 300;\n}\n[_v-2e2ce392]::-webkit-scrollbar {\n  display: none;\n}\n.xin-widget-citys[_v-2e2ce392] {\n  -webkit-transition: all 0.2s ease-out;\n  transition: all 0.2s ease-out;\n  position: fixed;\n  top: 0;\n  bottom: 0;\n  left: 0;\n  right: 0;\n  background: #fff;\n  z-index: 10000;\n  color: #333;\n  overflow-y: scroll;\n  -webkit-overflow-scrolling: touch;\n  text-align: left;\n  box-shadow: 1rem 1rem rgba(0, 0, 0, 0.2);\n  /***** 1px border *****/\n}\n.xin-widget-citys .xin-widget-citys-content[_v-2e2ce392] {\n  width: 100%;\n  height: auto;\n  background: #fff;\n  border-radius: 0.3rem;\n}\n.xin-widget-citys .xin-widget-citys-content .xin-widget-citys-header[_v-2e2ce392] {\n  height: 3rem;\n  display: -webkit-box;\n  background: #fafafa;\n}\n.xin-widget-citys .xin-widget-citys-content .xin-widget-citys-header .xin-widget-citys-iptbox[_v-2e2ce392] {\n  -webkit-box-flex: 1.0;\n  height: 100%;\n  padding: 0 1.2rem;\n  border: 1px solid #f3f2f2;\n}\n.xin-widget-citys .xin-widget-citys-content .xin-widget-citys-header .xin-widget-citys-iptbox input[_v-2e2ce392] {\n  width: 100%;\n  height: 100%;\n  /*line-height: 0.38rem;*/\n  display: block;\n  border: none;\n  background: none;\n  box-sizing: border-box;\n}\n.xin-widget-citys .xin-widget-citys-content .xin-widget-citys-header .xin-widget-citys-iptbox input[_v-2e2ce392]::-webkit-input-placeholder {\n  /*color: #cdcdcd;*/\n  text-align: left;\n}\n.xin-widget-citys .xin-widget-citys-content .xin-widget-citys-header a[_v-2e2ce392] {\n  width: 5rem;\n  height: 100%;\n  padding-top: 0.7rem;\n  /*line-height: 0.38rem;*/\n  text-align: center;\n  color: #999999;\n  display: block;\n}\n.xin-widget-citys .xin-widget-citys-content .xin-widget-citys-local[_v-2e2ce392] {\n  height: 3rem;\n  line-height: 3rem;\n  text-indent: 1.2rem;\n}\n.xin-widget-citys .xin-widget-citys-content .xin-widget-citys-list[_v-2e2ce392] {\n  position: relative;\n}\n.xin-widget-citys .xin-widget-citys-content .xin-widget-citys-list dl dt[_v-2e2ce392] {\n  line-height: 2rem;\n  color: #878787;\n  background: #f0f0f0;\n  text-indent: 0.5rem;\n}\n.xin-widget-citys .xin-widget-citys-content .xin-widget-citys-list dl dt em[_v-2e2ce392] {\n  position: relative;\n  top: 0.1rem;\n  margin-right: 6px;\n}\n.xin-widget-citys .xin-widget-citys-content .xin-widget-citys-list dl dd[_v-2e2ce392] {\n  padding: 0 1rem;\n  height: 2.3rem;\n  line-height: 2.3rem;\n}\n.xin-widget-citys .xin-widget-citys-searchlist[_v-2e2ce392] {\n  position: absolute;\n  top: 4rem;\n  width: 100%;\n  bottom: 0;\n  z-index: 10001;\n  background: #fff;\n  overflow: auto;\n}\n.xin-widget-citys .xin-widget-citys-searchlist li[_v-2e2ce392] {\n  margin: 0 1.6rem;\n  height: 2.5rem;\n  line-height: 2.5rem;\n  /*line-height: 0.5rem;*/\n}\n.xin-widget-citys .xin-widget-citys-searchlist .nomatch[_v-2e2ce392] {\n  /*line-height: 0.5rem;*/\n  text-align: center;\n  color: #aaa;\n}\n.xin-widget-citys .bdt[_v-2e2ce392],\n.xin-widget-citys .bdb[_v-2e2ce392],\n.xin-widget-citys .bdtb[_v-2e2ce392],\n.xin-widget-citys .bdl[_v-2e2ce392],\n.xin-widget-citys .bdr[_v-2e2ce392],\n.xin-widget-citys .bdlr[_v-2e2ce392] {\n  position: relative;\n}\n.xin-widget-citys .bdl[_v-2e2ce392]:before,\n.xin-widget-citys .bdr[_v-2e2ce392]:after,\n.xin-widget-citys .bdlr[_v-2e2ce392]:before,\n.xin-widget-citys .bdlr[_v-2e2ce392]:after,\n.xin-widget-citys .bdt[_v-2e2ce392]:before,\n.xin-widget-citys .bdb[_v-2e2ce392]:after,\n.xin-widget-citys .bdtb[_v-2e2ce392]:before,\n.xin-widget-citys .bdtb[_v-2e2ce392]:after {\n  background-size: 100% 100%;\n  position: absolute;\n  content: '';\n  z-index: 10;\n}\n.xin-widget-citys .bdl[_v-2e2ce392]:before,\n.xin-widget-citys .bdr[_v-2e2ce392]:after,\n.xin-widget-citys .bdlr[_v-2e2ce392]:before,\n.xin-widget-citys .bdlr[_v-2e2ce392]:after {\n  top: 0;\n  width: 1px;\n  height: 100%;\n}\n.xin-widget-citys .bdt[_v-2e2ce392]:before,\n.xin-widget-citys .bdb[_v-2e2ce392]:after,\n.xin-widget-citys .bdtb[_v-2e2ce392]:before,\n.xin-widget-citys .bdtb[_v-2e2ce392]:after {\n  left: 0;\n  height: 1px;\n  width: 100%;\n}\n.xin-widget-citys .bdb[_v-2e2ce392]:after {\n  background-image: url('data:image/png;base64,');\n  bottom: 0;\n}\n.xin-widget-citys-letnav[_v-2e2ce392] {\n  position: fixed;\n  right: 0;\n  bottom: 0;\n  top: 0.48rem;\n  padding-right: 0.26rem;\n  z-index: 10002;\n  display: -webkit-box;\n  -webkit-box-align: center;\n}\n.xin-widget-citys-letnav section[_v-2e2ce392] {\n  -webkit-box-flex: 1.0;\n}\n.xin-widget-citys-letnav ol[_v-2e2ce392] {\n  text-align: right;\n  padding-bottom: 0.48rem;\n}\n.xin-widget-citys-letnav ol li[_v-2e2ce392] {\n  padding: 0.03rem 0.02rem;\n  color: #666666;\n  width: 100%;\n  line-height: 100%;\n  box-sizing: border-box;\n  text-align: center;\n}\n.slide-enter[_v-2e2ce392],\n.slide-leave-active[_v-2e2ce392] {\n  -webkit-transform: translateX(100%);\n          transform: translateX(100%);\n}\n.fade-enter[_v-2e2ce392],\n.fade-leave-active[_v-2e2ce392] {\n  opacity: 0;\n}\n.star-big[_v-2e2ce392] {\n  width: 1rem;\n  height: 1rem;\n  display: inline-block;\n  background-size: cover;\n  background-image: url('data:image/png;base64,');\n}\n.star-small[_v-2e2ce392] {\n  width: 1.2rem;\n  height: 1.2rem;\n  display: inline-block;\n  background-size: cover;\n  background-image: url('data:image/png;base64,');\n}\n"] = false
    document.head.removeChild(__vueify_style__)
  })
  if (!module.hot.data) {
    hotAPI.createRecord("_v-2e2ce392", module.exports)
  } else {
    hotAPI.update("_v-2e2ce392", module.exports, (typeof module.exports === "function" ? module.exports.options : module.exports).template)
  }
})()}